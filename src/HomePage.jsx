...
import { loadCircleButtons } from './ConfigService';
...
export default function HomePage() {
  ...
  // ✅ Load circle buttons from Supabase (with local fallback)
  useEffect(() => {
    (async () => {
      try {
        ...
        if (buttons && buttons.length > 0) {
          ...
          // Default buttons if none saved anywhere
          setCircleButtons([
            { id: 1, title: 'Official Website', description: 'Visit our main website', image: '', link: 'https://example.com', type: 'website' },
            { id: 2, title: 'Rules PDF', description: 'Download competition rules', image: '', link: '', type: 'pdf' },
            { id: 3, title: 'Training Center', description: 'Find training locations', image: '', link: 'https://maps.google.com', type: 'website' },
            { id: 4, title: 'Registration Form', description: 'Download registration form', image: '', link: '', type: 'pdf' },
            { id: 5, title: 'Contact Us', description: 'Get in touch with us', image: '', link: 'mailto:contact@judoalgeria.com', type: 'website' }
          ]);
        }
      } catch (error) {
  ...
  // ✅ Sync circle buttons and announcements across tabs/windows
  useEffect(() => {
    const handleStorage = (e) => {
      if (e.key === 'algerian_judo_circle_buttons') {
        try {
          const parsed = e.newValue ? JSON.parse(e.newValue) : [];
          setCircleButtons(parsed || []);
        } catch (err) {
          console.error('Error parsing circle buttons from storage event:', err);
        }
      }
      if (e.key === 'algerian_judo_banner_announcements') {
        try {
          const parsed = e.newValue ? JSON.parse(e.newValue) : [];
          setAnnouncements(parsed || []);
        } catch (err) {
          console.error('Error parsing announcements from storage event:', err);
        }
      }
    };
