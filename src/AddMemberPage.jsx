...
export default function AddMemberPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Federation President-only rule
      if ((selectedFederationRole || '').trim() !== 'Federation President') {
        setError('Federation Role must be Federation President');
        setSubmitLoading(false);
        return;
      }

      // Prevent multiple presidents
      const { data: presidentRows, error: presidentErr } = await supabase
        .from('members')
        .select('id')
        .eq('role', 'Federation President')
        .limit(1);
      if (presidentErr) {
        setError(`Failed to validate president role: ${presidentErr.message}`);
        setSubmitLoading(false);
        return;
      }
      if (presidentRows && presidentRows.length > 0) {
        setError('The president of the federation is already exist.');
        setSubmitLoading(false);
        return;
      }
