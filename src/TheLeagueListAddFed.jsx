...
const fetchAllMembers = async () => {
  try {
    ...
    if (error) {
      ...
      setMembers(data || []);
      // removed success message on initial load
    }
  } catch (err) {
    setError(err);
  } finally {
...
// Function to normalize image orientation using EXIF data with fallback
  function normalizeImage(url) {
   ...
   const exportPDFWithAutoOrientation = async (data) => {
    ...
    // Add title
    const title = `League Member Information - ${data.first_name} ${data.last_name}`;
    doc.setFontSize(16);
    doc.setTextColor(40, 40, 40);
...
// ------------------ upload helpers ------------------
  ...
  const getNextRenewal = async (nidVal, leagueIdVal, roleVal, currentSeasonYear, excludeMemberId = null) => {
    ...
    const { data: existingInCurrentYear } = await existingQuery;
    ...
    // Count total registrations across all years for this combination, excluding the current row if editing
  ...
  const handleSave = async (id) => {
    ...
    try {
      ...
      // Enforce same league_id for save
      const leagueIdForSave = editedMember.league_id ?? selectedLeagueId;
      const leagueIdNumeric = Number(leagueIdForSave);
      if (!leagueIdNumeric || isNaN(leagueIdNumeric)) {
        setError("Please select a valid league.");
        setSaveLoading(false);
        return;
      }
      ...
      const originalMember = members.find((m) => m.id === Number(id ?? editedMember.id));
      const isKeyFieldsUnchanged = !!originalMember &&
        (originalMember.national_id_number === editedMember.national_id_number) &&
        (Number(originalMember.league_id) === leagueIdNumeric) &&
      ...
      const targetId = id ?? editedMember.id;
      const targetIdNumeric = Number(targetId);
      if (!targetIdNumeric || isNaN(targetIdNumeric)) {
      ...
      setMembers((prev) => prev.map((m) => (m.id === targetIdNumeric ? { ...m, ...updatedMember } : m)));
      setEditingId(null);
