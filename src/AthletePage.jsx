...
import { initializePrimaryButtons, handlePrimaryButtonClick } from "./primaryButtonHandler";
    ...
    export default function AthletePage() {
      ...
      // Derive global disabled state from club president confirmation (club-level authoritative)
      useEffect(() => {
        let mounted = true;
        const deriveDisabled = async () => {
          try {
            // Prefer displayed club context over logged-in member club
            const effectiveClubId = clubId || selectedClubId || member?.club_id;
            if (!effectiveClubId) {
              if (mounted) setButtonsDisabled(false);
              return;
            }
            const { data, error } = await supabase
              .from("club_members")
              .select("id, role, confirmation, year")
              .eq("club_id", effectiveClubId)
              .ilike("role", "%president of the club%")
              .limit(1);
            if (error) {
              console.error("deriveDisabled error", error);
              if (mounted) setButtonsDisabled(false);
              return;
            }
            const president = Array.isArray(data) ? data[0] : null;
            const isUnconfirmed = president ? String(president.confirmation).trim().toLowerCase() !== 'true' : false;
            const currentYear = new Date().getFullYear();
            const seasonYear = `${currentYear}/${currentYear + 1}`;
            const mismatch = president ? String(president.year || '').trim() !== seasonYear : false;
            if (mounted) {
              setButtonsDisabled(isUnconfirmed || mismatch);
              setSeasonMismatch(mismatch);
            }
          } catch (err) {
            console.error("deriveDisabled exception", err);
            if (mounted) setButtonsDisabled(false);
          }
        };
        deriveDisabled();
        return () => { mounted = false; };
      }, [clubId, selectedClubId, member, roles]);
...
useEffect(() => {
 ...
 // Check if Last Name and Password combination already exists
    ...
    try {
      ...
      // Wrapper for consistency with other pages naming
      ...
      // Delete allowed only on the same day of registration, and not if confirmed (DB-backed check)
      const deleteAthlete = async (id) => {
        try {
          // Fetch fresh record to validate registration_date and confirmation from DB
          const { data: fresh, error: fetchErr } = await supabase
            .from("athletes")
            .select("id, registration_date, confirmation")
            .eq("id", id)
            .single();
          if (fetchErr) throw fetchErr;

          const today = new Date().toISOString().split("T")[0];
          if (!fresh?.registration_date || fresh.registration_date !== today) {
            setError("Deletion allowed only on the same day of registration.");
            return;
          }

          // Block deletion even on the same day if the athlete is confirmed
          if (fresh?.confirmation) {
            setError("You cannot delete on the same day if the athlete is confirmed.");
            return;
          }
            ...
            {member ? (
              <p>
                ...
                <strong>{(() => {
                  ...
                  const safeMemberRole = typeof member.club_role === "object"
                    ? member.club_role?.club_role
                    : member.club_role;
                  return foundRole?.club_role || safeMemberRole || member.role || "No role";
                })()}</strong>
              </p>
            ) : (
  ...
  List of Athletes
            ...
            <form onSubmit={handleSubmit} className="form-grid">
              ...
              <label>
                  ...
                  <small style={{ color: '#666' }}>Temporary: select brown or black belt until the DB constraint is updated.</small>
                </label>
              ...
              <div className="btn-row">
                <button type="submit" className="primary-b" disabled={buttonsDisabled}>
                  Save Athlete
                </button>
              </div>
            ...
            <table className="athlete-table">
              <thead>
                <tr>
                  ...
                  <th>conf_Request</th> {/* Non-editable: show confirmation_request status */}
              ...
              <tbody>
               {filteredAthletes.length === 0 ? (
                      ...
                      {/* conf_Request status (non-editable) */}
                      ...
                      <td>
                        {editingId === a.id ? (
                          <>
                            <button type="button" className="primary-S" onClick={handleSave} disabled={buttonsDisabled}>Save</button>
                            <button type="button" className="secondary-btn" onClick={cancelEdit} disabled={buttonsDisabled}>Cancel</button>
                          </>
                        ) : (
                          <>
                            <button type="button" className="primary-M" onClick={() => startEdit(a)} disabled={buttonsDisabled}>Modify</button>
                            <button type="button" className="secondary-btn" onClick={() => deleteAthlete(a.id)} disabled={buttonsDisabled}>Delete</button>
                            <button
                              type="button"
                              className={a.confirmation_request ? "cancel-btn" : "confirm-btn"}
                              disabled={buttonsDisabled}
                              onClick={() => {
                                if (buttonsDisabled) return;
                                const confirmMsg = a.confirmation_request
                                  ? "Cancel confirmation request for this athlete?"
                                  : "Request confirmation for this athlete?";
                                if (!window.confirm(confirmMsg)) return;
                                (async () => {
                                  try {
                                    // Fetch fresh status from DB to ensure latest values
                                    const { data: fresh, error: fetchErr } = await supabase
                                      .from("athletes")
                                      .select("confirmation, confirmation_request")
                                      .eq("id", a.id)
                                      .single();
                                    if (fetchErr) throw fetchErr;

                                    const nextValue = !fresh?.confirmation_request;
                                    const { error: updateErr } = await supabase
                                      .from("athletes")
                                      .update({ confirmation_request: nextValue })
                                      .eq("id", a.id);
                                    if (updateErr) throw updateErr;

                                    setAthletes(prev => prev.map(row => row.id === a.id ? { ...row, confirmation_request: nextValue } : row));
                                    await fetchData(); // ensure UI reflects latest DB state
                                    setSuccess(nextValue ? "✅ Confirmation requested." : "❌ Confirmation request canceled.");
                                    setTimeout(() => setSuccess(""), 2000);
                                  } catch (err) {
                                    setError(err.message || "Failed to update confirmation request");
                                  }
                                })();
                              }}
                            >
                              {a.confirmation_request ? "Cancel Request Confirmation" : "Request Confirmation"}
                            </button>
                            <button
                              type="button"
                              className="primary-M"
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  // Block renewal if an entry for this NID already exists in the current season
                                  const { data: existingInSeason, error: existingErr } = await supabase
                                    .from("athletes")
                                    .select("id")
                                    .eq("national_id_number", a.national_id_number)
                                    .eq("year", seasonYear);
                                  if (existingErr) throw existingErr;
                                  if (existingInSeason && existingInSeason.length > 0) {
                                    setError("we can not add this athlete because is ready existe");
                                    return;
                                  }

                                  // Compute next renewal count across all years for same NID
                                  const { count, error: countErr } = await supabase
                                    .from("athletes")
                                    .select("*", { count: "exact", head: true })
                                    .eq("national_id_number", a.national_id_number);
                                  if (countErr) throw countErr;
                                  const nextRenewal = (count || 0) + 1;
                                  ...
                                  const newAthlete = {
                                    ...
                                    password: a.password,
                                    license_number: `ATH-${currentYear}-${Date.now()}`,
                                    registration_date: new Date().toISOString().split("T")[0],
                                    photos_url: a.photos_url,
                                  ...
                                  setSuccess("Renewal created successfully.");
