              ...
              <tbody>
               {filteredAthletes.length === 0 ? (
                      ...
                      <td>
                        {editingId === a.id ? (
                          <>
                            <button type="button" className="primary-S" onClick={handleSave}>Save</button>
                            <button type="button" className="secondary-btn" onClick={cancelEdit}>Cancel</button>
                          </>
                        ) : (
                          <>
                            <button type="button" className="primary-M" onClick={() => startEdit(a)}>Modify</button>
                            <button type="button" className="secondary-btn" onClick={() => deleteAthlete(a.id)}>Delete</button>
                            <button
                              type="button"
                              className={(a.confirmation_request && !a.confirmation) ? "cancel-btn" : "confirm-btn"}
                              disabled={a.confirmation && a.confirmation_request}
                              onClick={() => {
                                ...
                                (async () => {
                                  try {
                                    // Fetch fresh status from DB to ensure authoritative gating
                                    const { data: fresh, error: fetchErr } = await supabase
                                      .from("athletes")
                                    ...
                                    // If athlete is confirmed and current request is active, block cancellation
                                    if (fresh?.confirmation && fresh?.confirmation_request) {
                                      setError("Cannot cancel confirmation request for a confirmed athlete.");
                                      return;
                                    }
