...
export default function TheClubListAddFed() {
  ...
  useEffect(() => {
    initializePrimaryButtons();
      ...
      // Function to normalize image orientation using EXIF data with fallback (mirrors TheClubListAddFedLeague)
      ...
      // Individual member PDF export with auto orientation and normalized photo (mirrors TheClubListAddFedLeague)
      const exportPDFWithAutoOrientation = async (member) => {
        try {
          ...
          doc.setFontSize(16);
          doc.setTextColor(40, 40, 40);
          doc.text(`Club Member Information - ${sanitizeForPDF(member.first_name)} ${sanitizeForPDF(member.last_name)}`, 14, 30);
          ...
          if (photoUrl) {
            try {
              ...
              console.error('Error adding normalized photo, attempting original:', error);
              try {
          ...
          const memberData = [
            ...
            ['Role', sanitizeForPDF(foundRole?.club_role) || 'No Role'],
          ...
          // Medical certificate and signatures
          ...
          try {
            ...
            const qrCodeDataURL = await QRCode.toDataURL(qrData, { width: 60, margin: 1, color: { dark: '#000000', light: '#FFFFFF' } });
